...
namespace ltm {
    namespace util {
        ...
        public:
            ...
            bool
            getParameter(std::string key, std::vector<std::string> &parameter, std::vector<std::string> default_value);
        };
        ...
        ParameterServerWrapper::getParameter(std::string key, std::vector<std::string> &parameter,
            ...
            if (priv.getParam(key, parameter)) {
                ...
                ROS_INFO_STREAM(
                        " - using custom string list for '" << key << "' which has (" << parameter.size()
                                                            << ") points");
                ret_val = true;
                ...
                parameter = default_value;
                ROS_WARN_STREAM(
                        " - using default string list for '" << key << "' which has (" << parameter.size()
                                                             << ") points");
                ret_val = false;
            }
            priv.setParam(key, parameter);
